...
public class DrawNoteCore : MonoBehaviour
{
    ...
    /// <summary>
    /// Mode to draw notes (Normal, Finger)
    /// </summary>
    ...
    private void TryDrawNote(DrawNoteType instanceType)
    {
        ...
        float PinchThreshold = 0.7f;
        if (HandJointUtils.TryGetJointPose(TrackedHandJoint.Wrist, Handedness.Any, out pose) == false ||  HandPoseUtils.CalculateIndexPinch(Handedness.Any) <= PinchThreshold)
        {
            curDrawIndex += 1;
            return;
        }
